...
namespace UnityEngine.Experimental.Rendering.HDPipeline
{
    ...
    public class GBufferManager : MRTBufferManager
    {
        ...
        public RenderTargetIdentifier[] GetBuffersRTI(FrameSettings frameSettings)
        {
            ...
            for (int i = 0; i < m_BufferCount; ++i)
            {
                if (m_GBufferUsage[i] == GBufferUsage.ShadowMask && !frameSettings.IsEnable(FrameSettingsField.ShadowMask))
                    continue; // Skip

                if (m_GBufferUsage[i] == GBufferUsage.LightLayers && !frameSettings.IsEnable(FrameSettingsField.LightLayers))
                    continue; // Skip
